...
const postRegistration = async(req,res,next) => {
    try {
        if (!req.body.email || req.body.email.length == 0) return next(new ApiError('Invalid email input', 400));
        if (!req.body.password || req.body.password.length < 8) return next(new ApiError('Invalid password input (note: password must be 8 characters long or longer)', 400));
